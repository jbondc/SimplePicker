...
This has been tested with Google Chrome and Apple Safari, but not Internet Explorer. If you have to support IE, my sympathies, but personally I've already wasted too much of my life on that "browser". While I would like to support Firefox, it's become too much of a moving target. Also, Firefox doesn't (yet) support the HTML5 range (slider) controls. Feel free to fork. :-) The slider controls aren't supported on the Android browser, either, though that is likely to change once the default browser on that platform becomes Chrome. 
